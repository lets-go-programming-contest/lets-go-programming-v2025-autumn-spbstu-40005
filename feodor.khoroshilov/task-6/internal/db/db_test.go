...
import (
	...
	"feodor.khoroshilov/task-6/internal/db"
...
func TestGetNames_Success(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"name"}).AddRow("Ivan").AddRow("Dog228")
	mock.ExpectQuery(queryNames).WillReturnRows(rows)

	names, err := service.GetNames()
	require.NoError(t, err)
	require.Equal(t, []string{"Ivan", "Dog228"}, names)
}
...
func TestGetNames_RowsError(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"name"}).AddRow("Sergay")
